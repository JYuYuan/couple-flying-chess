...
export const GameBoard = React.memo(function GameBoard({
  ...
  const getCellStyle = useMemo(
    () => (pathCell: PathCell, isRedOnCell: boolean, isBlueOnCell: boolean) => {
      const baseClasses = `relative z-[999] rounded-full flex flex-col items-center justify-center aspect-square rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-xl shadow-gray-200/40 dark:shadow-black/30`;

      switch (pathCell.type) {
        case 'start':
          return `${baseClasses} bg-gradient-to-br from-amber-400 via-orange-400 to-orange-500 border-2 border-orange-300/60 shadow-orange-300/50`;
        case 'end':
          return `${baseClasses} bg-gradient-to-br from-emerald-400 via-green-500 to-green-600 border-2 border-green-300/60 shadow-green-300/50`;
        case 'star':
  ...
  const getConnectionLines = useMemo(
    () => (pathCell: PathCell) => {
      ...
      const lineClass = `absolute -z-10 rounded-sm shadow-sm dark:bg-gradient-to-r dark:from-cyan-400 to-blue-500
        bg-gradient-to-r from-blue-500 to-indigo-600 opacity-70`;
      ...
      cellElements[`${pathCell.y}-${pathCell.x}`] = (
        ...
        >
          <div className="relative z-10 flex flex-col items-center justify-center space-y-1">
            {pathCell.type === 'start' && <Rocket size={18} className={getIconColor('start')} />}
            {pathCell.type === 'end' && <Trophy size={18} className={getIconColor('end')} />}
            {pathCell.type === 'star' && <Star size={16} className={getIconColor('star')} />}
            {pathCell.type === 'trap' && <Bomb size={16} className={getIconColor('trap')} />}
            {pathCell.type === 'path' && (
          ...
          {getConnectionLines(pathCell)}
           ...
           {isRedOnCell && (
            <motion.div
              ...
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className={`absolute rounded-full h-full w-full z-20 flex items-center justify-center w-10 h-10 rounded-2xl bg-gradient-to-br from-red-500 to-red-600 border-2 border-white/80 shadow-2xl backdrop-blur-sm ${
          ...
          {isBlueOnCell && (
            <motion.div
              ...
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className={`absolute rounded-full h-full w-full z-20 flex items-center justify-center w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 border-2 border-white/80 shadow-2xl backdrop-blur-sm ${
    ...
    <div className="relative w-full aspect-square">
      <div className="relative w-full h-full p-3 rounded-3xl shadow-2xl border transition-all duration-500 backdrop-blur-xl bg-gradient-to-br from-white/90 via-slate-50/80 to-indigo-50/90 border-white/30 shadow-gray-300/40 dark:bg-gradient-to-br dark:from-slate-900/90 dark:via-gray-800/80 dark:to-slate-900/90 dark:border-gray-700/30 dark:shadow-black/40">
        <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
          <div
            className="absolute -top-6 -right-6 w-20 h-20 rounded-full blur-2xl opacity-15 shadow-lg"
            style={{
              background:
          ...
          ></div>
          <div
            className="absolute -bottom-6 -left-6 w-24 h-24 rounded-full blur-2xl opacity-15 shadow-lg"
            style={{
              background:
