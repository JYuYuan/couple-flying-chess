...
export const createBoardPath = (): PathCell[] => {
  ...
  for (let segment = 0; segment < segmentCount; segment++) {
    ...
    const segmentEnd =
      segment === segmentCount - 1 ? availablePositions.length : (segment + 1) * segmentSize;
  ...
  let remainingStarsToPlace =
    numStars - Array.from(placedPositions).filter((pos) => path[pos].type === 'star').length;
  let remainingTrapsToPlace =
    numTraps - Array.from(placedPositions).filter((pos) => path[pos].type === 'trap').length;

  for (
    let i = 0;
    i < shuffledRemainingPositions.length &&
    (remainingStarsToPlace > 0 || remainingTrapsToPlace > 0);
    i++
  ) {
